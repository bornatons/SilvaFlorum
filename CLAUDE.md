...
- **Formular:** Versand über Formspree per `fetch` in `js/bestellen.js`.
  Der Platzhalter `FORMSPREE_ID` steht an zwei Stellen (`bestellen.html`
  Action-Attribut und `js/bestellen.js`); solange er nicht ersetzt ist, zeigt
  das Formular einen Hinweis statt zu senden.
...
- `impressum.html`, `datenschutz.html` – Grundtexte für eine Schweizer
  Kleinunternehmerin (revDSG); offene Angaben als `[TODO: …]` mit CSS-Klasse
  `.todo` markiert.
...
## Offene Punkte (Stand: Erstversion)

- `[TODO: …]`-Stellen: Kontaktangaben (Footer aller Seiten + Kontakt-Sektion
  auf `index.html`), Impressum, Datenschutz (inkl. Hosting-Anbieter, Datum).
- `FORMSPREE_ID` eintragen (siehe README «Formular aktivieren»).
- Beispielprodukte/-preise und Platzhalterbilder durch echte ersetzen.
